...
const ALLOWED_PANE_KINDS = new Set(['terminal', 'sync-to-studio']);
...
function normalizeKind(kind) {
  ...
  if (typeof kind !== 'string' || !ALLOWED_PANE_KINDS.has(kind)) {
    return { ok: false, error: 'Unsupported terminal kind' };
  }
  return { ok: true, kind };
}
...
function sanitizeTerminalSpawnOptions(opts, defaults) {
  ...
  const kindResult = normalizeKind(opts.kind);
